...
const App = () => {
  ...
  const initAppCloud = async () => {
    ...
    try {
      ...
      await Promise.all([
          fetchInvoicesCloud(),
          fetchPurchasesCloud(),
          fetchProductsCloud(), // Adicionado para alimentar o formulário
      ...
      if (error) {
        ...
        const mapped: Invoice[] = data.map(f => {
          ...
          return {
            ...
            currency: 'AOA',
            exchangeRate: 1,
            // CORREÇÃO: Não certificar documentos pendentes automaticamente
            status: f.status === 'Pago' ? InvoiceStatus.PAID : f.status === 'Anulado' ? InvoiceStatus.CANCELLED : InvoiceStatus.PENDING,
            isCertified: f.status === 'Pago' || f.status === 'Anulado' || (f.numero_fatura && f.numero_fatura !== 'DRAFT'),
            hash: '',
  ...
  const getNextSequenceFromCloud = async (prefix: string, companyId: string): Promise<number> => {
      const { data } = await supabase
          .from('faturas')
          .select('numero_fatura')
          .eq('tipo_fatura', prefix)
          .eq('empresa_id', companyId);
      
      let maxSeq = 0;
      if (data) {
          data.forEach(doc => {
              const parts = doc.numero_fatura.split('/');
  ...
  const handleSaveInvoice = async (inv: Invoice, sId: string, action?: string) => {
      ...
      if (!inv.isCertified || inv.number === 'DRAFT' || inv.number === 'POS-Gen') {
          const prefix = getDocumentPrefix(inv.type);
          const nextSeq = await getNextSequenceFromCloud(prefix, companyIdToUse!);
          
          const number = `${prefix} ${docSeries?.code || 'S'} ${docSeries?.year}/${nextSeq}`;
          finalInv = { ...inv, number, seriesCode: docSeries?.code };
          
          if (docSeries) {
              const updatedSequences = { ...docSeries.sequences, [inv.type]: nextSeq };
      ...
      try {
        ...
        // CORREÇÃO: Lógica de baixa de stock ao certificar
        if (finalInv.isCertified && finalInv.type !== InvoiceType.PP && finalInv.type !== InvoiceType.OR) {
          for (const item of finalInv.items) {
            if (item.productId) {
              await supabase.from('movimentos_stock').insert({
                ...
                armazem_id: finalInv.targetWarehouseId || null,
                documento_ref: finalInv.number,
                notas: `Baixa automática via ${finalInv.type}`,
                empresa_id: companyIdToUse
              });
            }
          }
  ...
  const handleDeleteInvoice = async (id: string) => {
      ...
      try {
          ...
          console.error("Erro ao apagar fatura da Cloud:", err);
      }
  };
  ...
  const handleLiquidate = async (invoice: Invoice, amount: number, method: PaymentMethod, registerId: string, dateValue: string, docDate: string) => {
      ...
      const prefix = 'RC';
      const nextSeq = await getNextSequenceFromCloud(prefix, companyIdToUse!);
      const number = `${prefix} ${docSeries.year}/${nextSeq}`;
    ...
    switch (currentView) {
      ...
      case 'SETTINGS':
        return <Settings 
                  ...
                  onDeleteCashRegister={id => setCashRegisters(cashRegisters.filter(x => x.id !== id))}
               />;
